...
from tradebot_util.backtest_v2 import run_backtest_v2, save_backtest_v2
...
def cmd_rank(args):
    ...
    print(f"Regime V2: {regime.regime} | Exposição alvo em ações: {regime.equity_exposure:.0%}")
...
def cmd_backtest(args):
    ...
    tickers_list = tickers(assets)
    prices = load_prices(config, args, tickers_list)
    result = run_backtest_v2(prices, assets, config)
    output_dir = args.output_dir or config.get("backtest", {}).get("output_dir", "backtests/results_v2")
    out = save_backtest_v2(result, output_dir)
    print("Métricas do backtest V2")
...
def build_parser():
    ...
    show_config.add_argument("--config", default="config_v2.yaml")
    show_config.set_defaults(func=cmd_show_config)

    rank = subparsers.add_parser("rank")
    rank.add_argument("--config", default="config_v2.yaml")
    rank.add_argument("--prices-csv", default=None)
    rank.set_defaults(func=cmd_rank)

    backtest = subparsers.add_parser("backtest")
    backtest.add_argument("--config", default="config_v2.yaml")
