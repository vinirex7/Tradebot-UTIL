...
def cmd_rank(args):
    ...
    print(f"Regime V4: {regime.regime} | Exposição base em ações: {regime.equity_exposure:.0%}")
...
def cmd_backtest(args):
    ...
    output_dir = args.output_dir or config.get("backtest", {}).get("output_dir", "backtests/results_v4")
    out = save_backtest_v4(result, output_dir)
    print("Métricas do backtest V4")
    ...
    print(f"\nArquivos V4 salvos em: {out}")


def build_parser():
    parser = argparse.ArgumentParser(description="Tradebot UTIL V4")
    subparsers = parser.add_subparsers(dest="command", required=True)

    show_config = subparsers.add_parser("show-config")
    show_config.add_argument("--config", default="config_v4.yaml")
    show_config.set_defaults(func=cmd_show_config)

    rank = subparsers.add_parser("rank")
    rank.add_argument("--config", default="config_v4.yaml")
    rank.add_argument("--prices-csv", default=None)
    rank.set_defaults(func=cmd_rank)

    backtest = subparsers.add_parser("backtest")
    backtest.add_argument("--config", default="config_v4.yaml")
